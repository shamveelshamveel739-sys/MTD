...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.prev_R2V_Mappings = {}  # Preserve previous mapping
    ...
    def update_resources(self, ev):
        ...
        self.prev_R2V_Mappings = self.R2V_Mappings.copy()  # Save old mapping
        ...
        for curSwitch in self.datapaths:
            ...
            # Grace period rules: allow both old and new virtual IPs
            for real_ip, prev_virtual_ip in self.prev_R2V_Mappings.items():
                current_virtual_ip = self.R2V_Mappings[real_ip]
                # For ICMP
                match_prev = parser.OFPMatch(eth_type=0x0800, ipv4_src=prev_virtual_ip)
                actions_prev = [parser.OFPActionSetField(ipv4_src=current_virtual_ip), parser.OFPActionOutput(ofProto.OFPP_CONTROLLER, ofProto.OFPCML_NO_BUFFER)]
                self.add_flow(curSwitch, 1, match_prev, actions_prev, hard_timeout=10)
                # For ARP
                match_prev_arp = parser.OFPMatch(eth_type=0x0806, arp_spa=prev_virtual_ip)
                actions_prev_arp = [parser.OFPActionSetField(arp_spa=current_virtual_ip), parser.OFPActionOutput(ofProto.OFPP_CONTROLLER, ofProto.OFPCML_NO_BUFFER)]
                self.add_flow(curSwitch, 1, match_prev_arp, actions_prev_arp, hard_timeout=10)
    ...
    def handlePacketInEvents(self, ev):
        ...
        if arp_Obj:
            ...
            if self.isVirtualIPAddress(dst):
                ...
                # Use current or previous mapping if needed
                if dst in self.V2R_Mappings:
                    real_dst = self.V2R_Mappings[dst]
                else:
