...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.logger.info("Switch connected and table-miss flow added")
    ...
    def handle_arp(self, datapath, in_port, eth, a):
        ...
        if a.opcode == arp.ARP_REQUEST:
            if a.dst_ip in self.reverse_mapping:
                ...
                self.logger.info(f"Replied ARP: virt={a.dst_ip} -> real={real_ip}")
    ...
    def shuffle_mappings(self):
        ...
        self.logger.info("Cleared old flows")
    ...
    def send_gratuitous_arp(self, datapath, real_ip, virt_ip, mac, ports):
        ...
        for p in ports:
            ...
            out = parser.OFPPacketOut(datapath=datapath,
                                      ...
                                      data=pkt.data)
            datapath.send_msg(out)
            self.logger.info(f"Sent GARP: {virt_ip} on port {p}")

    # ---------------------------------------------------------------------
    # Install bidirectional flow rules
    # ---------------------------------------------------------------------
    def install_static_flows(self):
        if not self.datapath or len(self.ports) < len(self.mapping):
            self.logger.warning("Ports not ready, skipping flow install")
            return

        parser = self.datapath.ofproto_parser
        ofproto = self.datapath.ofproto

        host_list = list(self.mapping.items())  # [(real, virt), ...]

        for i, (r1, v1) in enumerate(host_list):
            for j, (r2, v2) in enumerate(host_list):
                if i == j:
                    continue
                # Forward v1->v2
                match = parser.OFPMatch(eth_type=0x0800,
                                        ipv4_src=v1, ipv4_dst=v2)
                actions = [parser.OFPActionOutput(self.ports[j])]
                self.add_flow(self.datapath, 10, match, actions)
                # Reverse v2->v1
                match = parser.OFPMatch(eth_type=0x0800,
                                        ipv4_src=v2, ipv4_dst=v1)
                actions = [parser.OFPActionOutput(self.ports[i])]
                self.add_flow(self.datapath, 10, match, actions)

        self.logger.info("Installed bidirectional static flows for all virtual IPs")
